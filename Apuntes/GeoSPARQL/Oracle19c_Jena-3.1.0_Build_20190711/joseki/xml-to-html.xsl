...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="alternating" >1</xsl:variable>
  ...
  <xsl:template name="vb-result">
    ...
    <!--    <xsl:variable name="alternating" select="1-$alternating" /> -->
    <tr>
  ...
  <xsl:template match="res:uri">
    ...
    <!-- <xsl:element name="a">
      <xsl:attribute name="href">
         <xsl:text> books?query=SELECT ?p ?y WHERE {</xsl:text>
        <xsl:text>&lt;</xsl:text>
        <xsl:value-of select="$uri"/>
        <xsl:text>&gt;</xsl:text>
        <xsl:text> ?p ?y}</xsl:text>
        <xsl:text>&amp;</xsl:text>
        <xsl:text>stylesheet=/joseki/xml-to-html.xsl</xsl:text>
      </xsl:attribute>
      <xsl:value-of select="$uri"/>
    </xsl:element> -->
  </xsl:template>
  ...
  <xsl:template match="res:sparql">
    <html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
      <head>
        ...
        <title>Oracle SPARQL Endpoint Query Results</title>
    </head>
      <body>

  <h1>Oracle SPARQL Endpoint Query Results</h1>
  <br/><br/>
